...
describe('fetchProject', () => {
  it('simulates fetching of a project', () => {
    ...
    expect(objectReducer([], action)).toEqual(expected);
  });
});
...
describe('newObjectFromTemplate', () => {
  it('try to add a new Object from a template', () => {
    ...
    const expected = [
      ...objectReducerTestState().map((object) => {
        if (object.id === 0) {
          return {
            ...object,
...
describe('linkObject', () => {
  ...
  it('try to link Datastream from room1 to room2', () => {
    ...
    const expected = objectReducerTestState().map((object) => {
      if (object.id === 7) {
        return {
          ...object,
...
describe('changeProjectName', () => {
  it('tests changing of the Project name', () => {
    ...
    const expectedOutput = testState().fisaProject.objects.map((object) => {
